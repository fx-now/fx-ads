...
function login($usuario, $senha, $destino){
	...
	if(mysql_num_rows($sel) == 0){//se não existir, mostra a mensagem abaixo
		...
		ins("sessoes","ids, usuario, senha","'$idsession', '$usuario', '$senha'");
		echo "Redirecionando ... ";
...
function is_on(){
	...
	if(total($sel) == 0){
		...
		$ids = $_SESSION["idsession"];
		$sel = sel("sessoes","ids = '$ids' and usuario = '$login' and senha = '$senha'","","");
		if(total($sel) == 0){
			return false;
		}else{
			return true;
...
/**
 * Retorna a função mysql_fetch_array
 *
 * @name fetch
 * @param string $query
 * @return string
